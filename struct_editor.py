#!/usr/bin/env python3
"""
Interactive editor for structured typing bot files (.sxt).
...
Usage:
    python struct_editor.py [filename.sxt]
"""
...
class StructEditor:
    """Interactive editor for .sxt files with live preview."""
    ...
    def _filter_section_tags(self, lines: List[str]) -> List[str]:
        ...
        for line in lines:
            ...
            cleaned_line = re.sub(
                r"</?(?:CODE|COMMANDS)(?:\s*:\s*[A-Z]+)?\s*>", "", line
            )
            if cleaned_line or (
                not line.strip().startswith("<") or not line.strip().endswith(">")
            ):
    ...
    def _handle_arrow_command(
        self, cmd: str, text_buffer: List[str], cursor_line: int, cursor_col: int
    ) -> tuple:
        ...
        if cmd == "u":  # Up
            ...
            if cursor_line < len(text_buffer) and cursor_col < len(
                text_buffer[cursor_line]
            ):
                cursor_col += 1
    ...
    def get_syntax_highlight_attr(self, line: str, x: int) -> int:
        ...
        if line_stripped.startswith("<") and (
            "CODE" in line_stripped or "COMMANDS" in line_stripped
        ):
            if ":" in line_stripped:
                # Tool specifier
    ...
    def get_python_syntax_highlight(
        self, line: str, start_pos: int, length: int
    ) -> List[tuple]:
        ...
        while i < len(text):
            ...
            if text[i].isalpha() or text[i] == "_":
                ...
                elif word in python_builtins or (
                    next_non_space < len(text) and text[next_non_space] == "("
                ):
    ...
    def draw_line(self, y: int, line: str, is_current: bool = False):
        ...
        try:
            if use_python_highlighting:
                ...
                highlighted_parts = self.get_python_syntax_highlight(
                    visible_line, 0, len(visible_line)
                )
                x_pos = 4
                for text_part, color_attr in highlighted_parts:
                    if x_pos >= editor_width:
                        break
                    display_text = (
                        text_part[: editor_width - x_pos]
                        if x_pos + len(text_part) > editor_width
                        else text_part
                    )
                    if display_text:
                        if is_current:
                            self.stdscr.addstr(
                                y, x_pos, display_text, color_attr | curses.A_BOLD
                            )
                        else:
    ...
    def draw_preview(self):
        ...
        try:
            ...
            self.stdscr.addstr(
                0, preview_x, header[:preview_width], self.COLOR_PREVIEW | curses.A_BOLD
            )
        except curses.error:
            pass
        ...
        for i, line in enumerate(preview_lines[self.preview_scroll :]):
            ...
            try:
                ...
                if "│" in display_line:
                        ...
                        if part:
                            ...
                            for text_part, color_attr in highlighted_parts:
                                if text_part:
                                    try:
                                        self.stdscr.addstr(
                                            screen_y, x_pos, text_part, color_attr
                                        )
                                        x_pos += len(text_part)
                                    except curses.error:
                                        pass
                    ...
                    highlighted_parts = self.get_python_syntax_highlight(
                        display_line, 0, len(display_line)
                    )
                    x_pos = preview_x
                    for text_part, color_attr in highlighted_parts:
                        if text_part:
    ...
    def draw_status(self):
        ...
        right_status = (
            "Ctrl+S:Save | Ctrl+O:Open | Ctrl+N:New | Ctrl+Q:Quit | ↑↓:Navigate"
        )
        ...
        if self.status_message:
            try:
                ...
                self.stdscr.clrtoeol()
                self.stdscr.addstr(
                    msg_y, 0, self.status_message[:max_x], self.COLOR_ERROR
                )
            except curses.error:
                pass
    ...
    def handle_key(self, key: int) -> bool:
        ...
        if key == curses.KEY_UP:
            ...
            if self.modified:
                self.status_message = (
                    "Unsaved changes! Save first or press Ctrl+Q again to force quit."
                )
                return True
            return False
    ...
    def run(self):
        ...
        while True:
            ...
            needs_redraw = (
                self.cursor_y != prev_cursor_y  # Cursor moved to different line
                or self.cursor_x
                != prev_cursor_x  # Cursor moved horizontally (affects preview)
                or self.scroll_y != prev_scroll_y  # Vertical scroll changed
                or self.scroll_x != prev_scroll_x  # Horizontal scroll changed
                or self.modified != prev_modified  # Content was modified
                or self.status_message != prev_status_message  # Status message changed
            )
...
def main():
    ...
    parser.add_argument(
        "filename",
        nargs="?",
        help="Path to the .sxt file to edit",
    )
